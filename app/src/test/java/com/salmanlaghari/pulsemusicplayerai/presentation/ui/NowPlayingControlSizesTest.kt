...
/**
 * Keeps the Now Playing player controls honest about sizing. Exact spec:
 *  - Previous/Next: 90dp container, 45dp icon.
 *  - Play/Pause:    128dp container, 58dp icon, filled circular primary control.
...
class NowPlayingControlSizesTest {
    ...
    fun prevNextExactSpec() {
        assertEquals("Prev/Next container must be 90dp", 90, NowPlayingControlSizes.PREV_NEXT_SIZE_DP)
        assertEquals("Prev/Next icon must be 45dp", 45, NowPlayingControlSizes.PREV_NEXT_ICON_DP)
        assertTrue(
    ...
    fun playPauseExactSpec() {
        assertEquals("Play/Pause container must be 128dp", 128, NowPlayingControlSizes.PLAY_PAUSE_SIZE_DP)
        assertEquals("Play/Pause icon must be 58dp", 58, NowPlayingControlSizes.PLAY_PAUSE_ICON_DP)
        assertTrue(
